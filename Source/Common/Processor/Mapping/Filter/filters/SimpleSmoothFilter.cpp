	...
	MappingFilter(getTypeString())
{
	...
	downSmooth = filterParams.addFloatParameter("Fall Smoothing", "If async, this is the smoothing when value is falling", .8f,0,1);
}
...
void SimpleSmoothFilter::onContainerParameterChangedInternal(Parameter * p)
{
	if (p == async)
	{
